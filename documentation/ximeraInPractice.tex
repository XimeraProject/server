...
two files, \code{firstActivity.tex} and \code{firstActivitySource.xim}.
...
.2 firstActivity.
.3 firstActivity.tex.
.3 firstActivitySource.xim.
}

The file \code{firstActivity.tex} is simply a wrapper for \code{firstActivitySource.xim} and contains the following code:

\begin{Verbatim}
  \documentclass{../ximera}
  \begin{document}
  \input{firstActivitySource.xim}
  \end{document}
\end{Verbatim}

On the other hand, \code{firstActivitySource.xim} should look something
like: 

\begin{Verbatim}
  +color(red)\headline{An example of syntax.} 
...
include a headline, title, prerequisites, and outcomes.  The headline
should be a one sentence description of the activity, and must come
before the title. Each prerequisite and outcome should be without
spaces, and every attempt should be made to use already available tags
for the prerequisites and outcomes needed/delivered by this
activity. These may appear anywhere in the document.
...
Each of these environments (except for hint) can have space after it in 
...
.3 secondActivitySource.xim.
.2 thirdActivity.
.3 thirdActivity.tex.
.3 thirdActivitySource.xim.
}
...
\begin{Verbatim}
  ...
  \maketitle
  \tableofcontents
  \input{./firstActivity/firstActivitySource.xim}
  \input{./secondActivity/secondActivitySource.xim}
  \input{./thirdActivity/thirdActivitySource.xim}
\end{document}
\end{Verbatim}
